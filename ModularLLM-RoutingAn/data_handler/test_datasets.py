...
def read_test_dataset_lang(ds_name, lang):
    ...
    if ds_name == 'arc-challenge':
        ds = load_dataset("alexandrainst/m_arc", lang, split='train', trust_remote_code=True)
    elif ds_name == 'hswag':
        ds = load_dataset("alexandrainst/m_hellaswag", lang, split='val', trust_remote_code=True)
    # elif ds_name == 'mgsm':
    #     ds = load_dataset("juletxara/mgsm", lang, split='train', trust_remote_code=True)
    elif ds_name == 'xnli':
        ds = load_dataset("facebook/xnli", lang, split='train', trust_remote_code=True)
    elif ds_name == 'mmlu':
        ds = load_dataset("alexandrainst/m_mmlu", lang, split='train', trust_remote_code=True)
...
def load_bbh_dataset():
    ...
    for task in bbh_subsets:
        ds = load_dataset('maveriq/bigbenchhard', task, split='train', trust_remote_code=True)
...
def read_test_dataset(ds_name):
    ...
    # https://huggingface.co/datasets/allenai/swag
        ...
        ds = load_dataset('allenai/swag', split='validation', trust_remote_code=True)
    ...
    # https://huggingface.co/datasets/google-research-datasets/mbpp
        ...
        ds = load_dataset('google-research-datasets/mbpp', split='full', trust_remote_code=True)
    elif ds_name == 'flan':
        ds = load_dataset("TahaBa/flan-routing-MoE-dataset", trust_remote_code=True)['test']
    else:
        raise f"Dataset {ds_name} is not supported yet."

...
class ArrowLoraLinearLayer(nn.Module):
    ...
    def __init__(self, in_features, arrow_config):
        ...
        )  # Set in create_arrow_m odel() with this format: task_0, task_1, ...
        ...
        # --- START: Change ---
        self.k_max = arrow_config.arrow_k_max
        self.num_experts = len(self.task_adapter_names)
        if self.k_max is None:
            raise ValueError("ArrowLoraLinearLayer requires 'arrow_k_max' to be set in ArrowConfig.")
        
        self.register_buffer(
            "prototypes",
            torch.empty((self.num_experts, self.k_max, in_features)),
            persistent=False
        )

        self.remove_intruders = arrow_config.remove_intruders
        self.intruder_threshold = arrow_config.intruder_threshold


        self.routing_callback = arrow_config.routing_callback
        self.top_k_callback = arrow_config.top_k_callback

        if self.routing_callback is not None:
            if self.top_k_callback is None:
                raise ValueError("If `routing_callback` is provided, `top_k_callback` must also be set in LoraConfig.")
            if self.top_k_callback > self.num_experts:
                raise ValueError(
                    f"`top_k_callback` ({self.top_k_callback}) cannot be greater than `num_experts` ({self.num_experts})."
                )
        # --- END: Change --- #

    @torch.no_grad()
    def _identify_intruders(self, all_vhs: torch.Tensor) -> torch.Tensor:
        """
        Identifies outlier singular vectors based on cosine similarity.
        Args:
            all_vhs (torch.Tensor): A tensor of shape (num_experts, rank, in_features)
                                     containing singular vectors for all experts.
        Returns:
            torch.Tensor: A boolean tensor of shape (num_experts, rank) where True indicates
                          an intruder vector.
        """
        E, R, F_in = all_vhs.shape
        is_intruder = torch.zeros((E, R), dtype=torch.bool, device=all_vhs.device)

        for r_idx in range(R):
            vectors_at_rank_r = all_vhs[:, r_idx, :]  # Shape: (E, F_in)

            mean_vector = F.normalize(vectors_at_rank_r.mean(dim=0), p=2, dim=0)

            similarities = F.cosine_similarity(
                F.normalize(vectors_at_rank_r, p=2, dim=1),
                mean_vector.unsqueeze(0)
            ) # Shape: (E,)

            is_intruder[:, r_idx] = similarities < self.intruder_threshold
    ...
    def forward(self, x, lora_A, lora_B, dropout, scaling):
        ...
        # # 1) similarity: sum over k of (h_in . v_k)^2
        # dot_products = torch.einsum("tf, ekf -> tek", tok, self.prototypes)
        
        # # Square and sum over the k dimension
        # sim = torch.sum(dot_products**2, dim=2)  # Shape: (t, E)
        # # --- END: First Change ---
        ...
        if self.routing_callback is not None:
            # Use torch.topk to get the k highest logit values and their indices
            top_k_logits, top_k_indices = torch.topk(sim, self.top_k_callback, dim=-1)

            # Call the user-provided function with the data

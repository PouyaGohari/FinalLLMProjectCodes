...
class ArrowConfig:
    ...
    remove_intruders: bool = field(
        default=False, 
        metadata={"help": "Enables the algorithm to remove intruder dimensions."},
    )
    intruder_threshold: float = field(
        default=0.5, 
        metadata={"help": "The threshhold to which we call a dimension an intruder which is between 0 and 1."},
    )
    arrow_k_max: int = field(
        default=1, 
        metadata={"help": "The 'k' in the top Vs in the SVD."}
    )
    routing_callback: Optional[Callable] = field(
        default=None,
        metadata={"help": "A callback function to receive top-k routing info from Arrow LoRA."},
    )
    top_k_callback: Optional[int] = field(
        default=None,
        metadata={"help": "The 'k' in top-k. The number of top experts to report to the routing_callback."},
    )
